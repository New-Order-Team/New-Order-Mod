...
function State_Init( message )
	...
	if message == OnUpdate then
		if (Object.Get_Hull() < 0.6) and (not ls_1) then
			spawn_point = Find_First_Object("GA_Teshic")
			user_faction = Find_Player("Empire")
			Create_Generic_Object("GA_Teshic_Last_Stand", spawn_point, user_faction)
			ls_1 = true
		end
		if (Object.Get_Hull() < 0.3) and (ls_1) and (not ls_2) then
			spawn_point = Find_First_Object("GA_Teshic")
			user_faction = Find_Player("Empire")
			previous_bonus = Find_First_Object("GA_Teshic_Last_Stand")
			previous_bonus.Despawn()
			Create_Generic_Object("GA_Teshic_Last_Stand2", spawn_point, user_faction)
			ls_2 = true
		end
		if (Object.Get_Hull() < 0.01) and (ls_1) and (ls_2) then
			previous_bonus2 = Find_First_Object("GA_Teshic_Last_Stand2")
